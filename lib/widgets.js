...
function renderWord(p) {
  ...
  const origin = wrapText(item.origin, 48, 2);
  return svgWrap(W, H, `
  <defs>${shadowFilter('sh', p.shadow)}</defs>
  <rect x="1" y="1" width="${W-2}" height="${H-2}" rx="${rx}" ry="${rx}" fill="${bgFill}" ${border} ${p.shadow ? 'filter="url(#sh)"' : ''}/>
  ...
  <text x="220" y="168" fill="${th.fg}" font-size="8" font-weight="800" letter-spacing="2" opacity="0.5">SYNONYMS</text>
  <text x="220" y="190" fill="${th.fg}" font-size="11" font-weight="700" opacity="0.8">${escXml(item.synonyms)}</text>
  ${p.showOrigin ? `
  <text x="358" y="168" fill="${th.fg}" font-size="8" font-weight="800" letter-spacing="2" opacity="0.5">ORIGIN</text>
  ${origin.map((line, i) => `<text x="358" y="${190+i*15}" fill="${th.fg}" font-size="10" font-weight="500" opacity="0.72">${escXml(line)}</text>`).join('')}
  ` : ''}
  `);
}
